...
void RMS_TO_DBFS(float spectrum[], float n_freq[], float dt, float smoothing_factor);
...
int main(void)
{
    ...
    float smoothing_factor  = 40.0f;
    float spectrum_scaling_factor = 3.0f;
    ...
    while (!WindowShouldClose()) {  // Detect window close button or ESC key
        ...
        if (IsFileDropped()) {
            ...
            if (droppedFiles.count > 0) {
                ...
                if (IsPathFile(file_path) && IsFileExtension(file_path, ".mp3")) {  // Check if file is a valid .mp3 file.
                    /**
        ...
        if(IsMusicReady(music_stream)) {
            ...
            //----------------------------------------------------------------------------------
            RMS_TO_DBFS(spectrum, n_freq, dt, smoothing_factor);
            //----------------------------------------------------------------------------------
            if (durations > 0.0f) {
...
void InitializeMusicInfo(const char *music_file_path, MusicInfo *music_info, Texture2D *album_cover_texture)
{
    ...
    Image image = LoadImageFromMemory(".jpeg", (unsigned char*) picture.data, picture.size);     // Loaded in CPU memory (RAM)
